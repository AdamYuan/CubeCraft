...
std::pair<std::vector<vert_block>, std::vector<vert_block>> World::GetMesh(glm::ivec3 chunkPos,
...
#define getBlockIVec3(pos) (getBlock((pos).x, (pos).y, (pos).z))
	...
	{
		posi = Chunk::GetPosFromNum(iterator);
		...
		for(short face=0; face<6; ++face)
		{
			...
			for(int vertex=0; vertex<4; ++vertex)
			{
				...
				block_lightings[Chunk::XYZ3(posi, CHUNK_SIZE)][face].AO[vertex] = b1 && b2 ? 0 : 3 - b1 - b2 - bc;
				...
				block_lightings[Chunk::XYZ3(posi, CHUNK_SIZE)][face].Light[vertex] = 15;//(light_t) (sum_li / sum);
			}
		}
	}
