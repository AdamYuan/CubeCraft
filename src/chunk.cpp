...
int chunk::getPosNum(int x, int y, int z)
{
	return x+CHUNK_SIZE*(y+CHUNK_SIZE*z);
}
int chunk::getPosNum(const glm::ivec3 &pos)
{
	return getPosNum(pos.x, pos.y, pos.z);
}
block chunk::get(int x,int y,int z)
{
	...
	return (block)blk[getPosNum(x, y, z)];
}
...
light_t chunk::getl(int x, int y, int z)
{
	...
	return lightMap[getPosNum(x, y, z)];
}
...
void chunk::updateLighting()
{
	...
	chunk *up_chk=parent->getChunk(chunkPos.x, chunkPos.y+1, chunkPos.z);
	if(up_chk)
		std::uninitialized_copy(&up_chk->beCoverd[0][0], &up_chk->beCoverd[CHUNK_SIZE-1][CHUNK_SIZE-1]+1, &be_coverd[0][0]);
	else
		std::uninitialized_fill(&be_coverd[0][0], &be_coverd[CHUNK_SIZE-1][CHUNK_SIZE-1]+1, false);

	/*for(int i=1;;++i)
	{
		chunk *up_chk=parent->getChunk(chunkPos.x, chunkPos.y+i, chunkPos.z);
		if(!up_chk)
			break;
...
void chunk::updateMeshing()
{
	...
	glm::ivec3 posi;
	for(posi.x=0;posi.x<CHUNK_SIZE;++posi.x)
		for(posi.y=0;posi.y<CHUNK_SIZE;++posi.y)
			for(posi.z=0;posi.z<CHUNK_SIZE;++posi.z)
			{
				...
				for(short face=0;face<6;++face)
				{
					...
					for(int v=0;v<4;++v)
					{
						block_lightings[chunk::getPosNum(posi)][face].ao[v]=
						...
						for(int n=0;n<3;++n)
						{
							sum+=block_m::isTransparent(neighboors[lookup3[face][v][n]]);
							sum_li+=neighboors_li[lookup3[face][v][n]];
						}
						block_lightings[chunk::getPosNum(posi)][face].light[v]=sum_li/sum;
					}
				}
			}
