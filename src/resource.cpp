...
namespace resource
{
	...
	void init()
	{
		...
		blockTexture.texParameter(GL_NEAREST, GL_NEAREST, GL_REPEAT);

		//init font textures
		fontTexture.load2d(TEXTURE_DIR + "font.png");
		fontTexture.buildMipmap();
		fontTexture.texParameter(GL_LINEAR_MIPMAP_LINEAR,GL_LINEAR,GL_REPEAT);

		fontBoldTexture.load2d(TEXTURE_DIR + "font_bold.png");
		fontBoldTexture.buildMipmap();
		fontBoldTexture.texParameter(GL_LINEAR_MIPMAP_LINEAR,GL_LINEAR,GL_REPEAT);
	}
